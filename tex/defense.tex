...
\begin{document}
	...
	\section{Backround}
		...
		\begin{frame}
			...
			\begin{description}
				\item[Unrestricted domain (universality)] \hfill \\
					All individual preferences are allowed.
				\item[Independence of irrelevant alternatives] \hfill \\
					If all voters' preferences between alternatives x and y remain the same, the group preference between x and y is unchanged even if voters change their preferences regarding other alternatives.
				\item[Pareto principle (unanimity)] \hfill \\
					If all individuals prefer x to y, then the group prefers x to y.
				\item[Non-dictatorship] \hfill \\
					There is no voter whose preference always dictates the group's preference.
			\end{description}
	...
	\subsection{Step 3}
		...
		\begin{frame}
			...
			\note{
				Unfortunately for me, but fortunately for the field of social choice theory as a whole.
			}
		\end{frame}
